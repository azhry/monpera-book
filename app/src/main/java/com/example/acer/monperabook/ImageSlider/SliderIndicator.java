...
/**
 * Created by acer on 19/12/2017.
 *
 * This class is used to make slide indicator
 */

public class SliderIndicator implements ViewPager.OnPageChangeListener {

    private static String TAG = "SliderIndicator";
...
//        }, 2500);
    ...
    private void initIndicators() {
        ...
        for (int i = 0; i < mPageCount; i++) {
            ...
            int dimen = mSize != 0 ? res.getDimensionPixelSize(mSize)
                    : ((int)res.getDisplayMetrics().density * defaultSizeInDp);
            int margin = mSpacing != 0 ? res.getDimensionPixelSize(mSpacing)
                    : ((int)res.getDisplayMetrics().density * defaultSpacingInDp);
